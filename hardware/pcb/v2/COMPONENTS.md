...
|-----|------|---------|----------|---------|
| J1 | 2x20 Female Header 2.54mm | THT (mounted on B.Cu side) | Pi Zero 2 W GPIO | The main interconnect between the carrier board and the Raspberry Pi. Carries: UART (GPIO14/15 for serial communication with RP2040), SWD (GPIO24/25 for firmware flashing), I2S (GPIO18-21 for audio codec data), I2C (GPIO2/3 for codec control), GPCLK0 (GPIO4 for optional MCLK), CODEC_RESET (GPIO22), +5V power to the Pi (pin 2), and GND. Mounted on the bottom side of the board so the Pi sits below. |
...
| J8 | JST ZH 4-pin SMD | JST_ZH_B4B-ZR-SM4-TF | Handset connector. Four-wire cable to the phone handset carrying mic signal (MIC_HOT/MIC_GND) and earpiece audio (EAR_P/EAR_N). The mic signal routes through J9 (kill switch) before reaching the codec. Earpiece audio comes directly from the codec's headphone amp in capless BTL mode (no coupling cap needed). |
